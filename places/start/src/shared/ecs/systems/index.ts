 ...
 * exist.
 */
export function start<T extends unknown[]>(container: Host, loop: Loop<T>, debug: Debugger<unknown[]>): void {
	...
	function load(module: ModuleScript, context: Context): void {
		if (module.Name.match("%.spec$")[0] !== undefined || module.Name.match("%.dev$")[0] !== undefined) return;
